(() => {
  ...
  if (!coarse && innerWidth > 1024 && !reduced) {
    ...
    $$("a,button,.glass,.flip-card,input,textarea").forEach(el => {
      el.addEventListener("mouseenter", () => { if (dot) { dot.style.width = "13px"; dot.style.height = "13px"; } if (ring) { ring.style.width = "42px"; ring.style.height = "42px"; ring.style.borderColor = "rgba(29,78,216,.45)"; } });
      el.addEventListener("mouseleave", () => { if (dot) { dot.style.width = "7px"; dot.style.height = "7px"; } if (ring) { ring.style.width = "28px"; ring.style.height = "28px"; ring.style.borderColor = "rgba(15,23,42,.25)"; } });
    });
  }

  /* ---------- Matrix rain with neon green and stars ---------- */
  ...
  if (matrix && !reduced) {
    ...
    const frame = () => {
      ...
      // Matrix rain with neon green glow
      ctx.shadowColor = '#00ff41';
      ctx.shadowBlur = 8;
      ctx.font = `${font}px "JetBrains Mono",monospace`;
      for (let i = 0; i < drops.length; i++) {
        const y = drops[i] * font;
        ctx.fillStyle = Math.random() > .94 ? "rgba(0,255,65,0.35)" : "rgba(0,255,65,0.8)";
      ...
      stars.forEach(star => {
        const twinkle = Math.sin(star.phase + performance.now() * 0.002) * 0.5 + 0.5;
        ctx.beginPath();
  ...
  const updateThreat = () => {
    ...
    threat?.classList.remove("low", "medium", "high");
    threat?.classList.add(level.toLowerCase());
    if (threat) threat.textContent = level;
    if (bar) { bar.style.width = n + "%"; bar.style.background = level === "LOW" ? "#059669" : level === "MEDIUM" ? "#b45309" : "#b91c1c"; }
  ...
  if (traffic) {
    ...
    const draw = (time = 0) => {
      ...
      ctx.lineWidth = 1.2;
      ctx.shadowBlur = 7;
      ctx.shadowColor = "rgba(29,78,216,.3)";
      ctx.stroke();
      ctx.shadowBlur = 0;
      data.push(Math.random() * .62 + .12);
      data.shift();
  ...
  if (log && !reduced) {
    const entries = [
      ["✓", "TLS handshake validated", "log-ok"],
      ["⚡", "Packet inspected (TLS 1.3)", "log-cyan"],
      ["✓", "Certificate validated (OCSP)", "log-ok"],
      ["⚠", "WAF blocked XSS attempt", "log-warn"],
      ["✓", "Ruleset synchronized", "log-ok"],
      ["✓", "Audit log rotated", "log-ok"],
      ["🔍", "Vulnerability scan completed", "log-cyan"],
      ["✓", "No anomaly detected", "log-ok"]
    ];
    ...
    setInterval(() => {
      ...
      d.className = cls;
      d.textContent = `[${new Date().toLocaleTimeString("id-ID", { hour12: false })}] ${icon} ${text}`;
      log.appendChild(d);
  ...
  form?.addEventListener("submit", async e => {
    ...
    submit.innerHTML = "<span>Memproses…</span>";
    status.textContent = "";
    try {
      ...
      link.href = mail;
      link.textContent = "Kirim via email";
  ...
  if (globeEl && innerWidth >= 1025 && !reduced) {
    const loadGlobe = async () => {
      try {
        ...
        const ring = new THREE.Mesh(new THREE.TorusGeometry(1.22, .008, 8, 80), ringMat);
        ring.rotation.x = .9;
        group.add(ring);
        const pointGeo = new THREE.SphereGeometry(.025, 6, 6);
        for (let i = 0; i < 22; i++) {
          const phi = Math.acos(1 - 2 * (i + .5) / 22), theta = Math.PI * (1 + Math.sqrt(5)) * i;
          const p = new THREE.Mesh(pointGeo, new THREE.MeshBasicMaterial({ color: i % 3 === 0 ? 0xb45309 : 0x1d4ed8 }));
          p.position.set(Math.cos(theta) * Math.sin(phi), Math.cos(phi), Math.sin(theta) * Math.sin(phi));
          group.add(p);
        }
        ...
        const animate = () => { if (running) { group.rotation.y += .0022; ring.rotation.z += .001; renderer.render(scene, camera); } requestAnimationFrame(animate); };
        animate();
      } catch (e) {
        globeEl.innerHTML = '<div style="height:100%;display:grid;place-items:center;color:#475569;font:9px JetBrains Mono,monospace">WEBGL FALLBACK · TELEMETRY READY</div>';
      }
    };
    if ("requestIdleCallback" in window) requestIdleCallback(loadGlobe, { timeout: 1800 }); else setTimeout(loadGlobe, 500);
  } else if (globeEl) {
    globeEl.innerHTML = '<div style="height:100%;display:grid;place-items:center;text-align:center;color:#475569;font:9px JetBrains Mono,monospace">MOBILE FALLBACK<br>3D TELEMETRY DISABLED FOR PERFORMANCE</div>';
  }
  ...
  /* ---------- GitHub live stats ---------- */
  const githubValue = $("#githubValue"), githubMeta = $("#githubMeta");
  const loadGithub = async () => {
    const user = window.RZ_CONFIG?.githubUsername;
    if (!user || !githubValue) return;
    try {
      const r = await fetch(`https://api.github.com/users/${user}`, { cache: "no-store" });
      if (!r.ok) throw new Error("github");
      const d = await r.json();
      githubValue.textContent = `${d.public_repos ?? 0} repo · ${d.followers ?? 0} follower`;
      if (githubMeta) githubMeta.textContent = `@${user} · sejak ${new Date(d.created_at).getFullYear()}`;
    } catch (e) {
      githubValue.textContent = "Data sementara offline";
      if (githubMeta) githubMeta.textContent = `@${user}`;
    }
  };
  loadGithub();
  setInterval(loadGithub, 15 * 60 * 1000);
  ...
  if (spiderCanvas && !reduced) {
    ...
    const resize = () => {
      ...
      spiderCanvas.width = Math.floor(W * dpr);
      spiderCanvas.height = Math.floor(H * dpr);
      spiderCanvas.style.width = W + "px";
      spiderCanvas.style.height = H + "px";
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      const count = W < 600 ? 18 : 34;
    ...
    const draw = (t = 0) => {
      ...
      for (let ring = 1; ring <= 4; ring++) {
        const r = base * ring / 2.2;
        ctx.beginPath();
        for (let i = 0; i <= 32; i++) {
          const a = (i / 32) * Math.PI * 2;
          const wob = Math.sin(a * 4 + t * .0004 + ring) * 1.5;
          const x = cx + Math.cos(a) * (r + wob), y = cy + Math.sin(a) * (r + wob);
          i ? ctx.lineTo(x, y) : ctx.moveTo(x, y);
        }
        ctx.closePath();
        ctx.strokeStyle = ring % 2 ? "rgba(29,78,216,.075)" : "rgba(220,38,38,.055)";
        ctx.lineWidth = .7;
        ctx.stroke();
      }
      for (let i = 0; i < 12; i++) {
        ...
        ctx.moveTo(cx, cy);
        ctx.lineTo(ex, ey);
        ctx.strokeStyle = i % 3 === 0 ? "rgba(220,38,38,.075)" : "rgba(29,78,216,.15)";
        ctx.stroke();
      }
      nodes.forEach((n, i) => {
        n.x += n.vx;
        ...
        ctx.beginPath();
        ctx.arc(n.x, n.y, n.r + Math.sin(n.phase) * .35, 0, Math.PI * 2);
        ctx.fillStyle = i % 5 === 0 ? `rgba(220,38,38,${alpha})` : `rgba(29,78,216,${alpha})`;
        ctx.fill();
        nodes.forEach((m, j) => {
          if (j <= i) return;
          ...
          if (dist < 130) {
            ...
            ctx.moveTo(n.x, n.y);
            ctx.lineTo(m.x, m.y);
            ctx.strokeStyle = `rgba(29,78,216,${(1 - dist / 130) * .045})`;
            ctx.lineWidth = .55;
            ctx.stroke();
          }
        });
    ...
    resize();
    addEventListener("resize", resize, { passive: true });
    document.addEventListener("visibilitychange", () => { running = !document.hidden; if (running) raf = requestAnimationFrame(draw); });
    raf = requestAnimationFrame(draw);
  }

  /* ---------- Hanging arachnid canvas ---------- */
  ...
  if (arachnidCanvas && !reduced) {
    ...
    function resize() {
      ...
      arachnidCanvas.width = W * dpr; arachnidCanvas.height = H * dpr;
      arachnidCanvas.style.width = W + 'px'; arachnidCanvas.style.height = H + 'px';
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      nodes.length = 0;
      for (let i = 0; i < 34; i++) nodes.push({ x: Math.random() * W, y: Math.random() * H, v: 0.08 + Math.random() * .18, p: Math.random() * Math.PI * 2 });
    }
    function line(x1, y1, x2, y2, a) { ctx.strokeStyle = `rgba(29,78,216,${a})`; ctx.lineWidth = .55; ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); ctx.stroke(); }
    function drawFigure(cx, top, scale, phase) {
      ...
      for (let i = 0; i < 3; i++) { ctx.strokeStyle = `rgba(29,78,216,${.10 - i * .02})`; ctx.lineWidth = .55; ctx.beginPath(); ctx.moveTo(i * 3 - 3, 0); ctx.lineTo(i * 2 - 2, headY - 10); ctx.stroke(); }
      ctx.fillStyle = 'rgba(9,13,18,.86)'; ctx.strokeStyle = 'rgba(220,38,38,.55)'; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.arc(0, headY, 12 * scale, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      ctx.strokeStyle = 'rgba(29,78,216,.30)'; ctx.lineWidth = .45; ctx.beginPath(); ctx.moveTo(-8 * scale, headY - 3); ctx.lineTo(8 * scale, headY + 5); ctx.moveTo(8 * scale, headY - 3); ctx.lineTo(-8 * scale, headY + 5); ctx.moveTo(-10 * scale, headY); ctx.lineTo(10 * scale, headY); ctx.stroke();
      ...
      ctx.strokeStyle = 'rgba(220,38,38,.32)'; ctx.lineWidth = 1.2; ctx.beginPath(); ctx.moveTo(0, shoulderY + 8); ctx.lineTo(0, shoulderY + 50); ctx.moveTo(-5, shoulderY + 24); ctx.lineTo(5, shoulderY + 24); ctx.stroke();
    ...
    function frame() {
      ...
      ctx.save();
      ctx.globalAlpha = .28;
      for (let k = 0; k < 7; k++) { const y = H * (.16 + k * .105) + Math.sin(t + k) * 10; ctx.beginPath(); ctx.strokeStyle = k % 2 ? 'rgba(29,78,216,.16)' : 'rgba(220,38,38,.12)'; ctx.lineWidth = .6; ctx.moveTo(W * .48, y); ctx.lineTo(W * .98, y - 50); ctx.stroke(); }
      ctx.restore();
      nodes.forEach(n => { n.p += n.v * .04; n.y += n.v; if (n.y > H + 10) n.y = -10; ctx.fillStyle = `rgba(29,78,216,${.12 + .12 * Math.sin(n.p)})`; ctx.fillRect(n.x, n.y, 1.3, 1.3); });
  ...
  const renderThreatFeed = (items, live) => {
    ...
    if (threatBadge) {
      ...
      label.textContent = live ? "LIVE THREATFOX" : "DEMO TELEMETRY";
      threatBadge.append(dot, label);
    }
  };
  const loadThreats = async () => {
    if (!threatMap) return;
    try {
      const endpoint = window.RZ_CONFIG?.threatApi || "/api/threats";
      const r = await fetch(endpoint, { cache: "no-store" });
      if (!r.ok) throw new Error("threat");
      const d = await r.json();
      if (d.live) {
        if (iocCount) iocCount.textContent = String(d.count ?? 0);
        if (malwareCount) malwareCount.textContent = String(d.families ?? 0);
        if (sourceAge) sourceAge.textContent = "< 24h";
        if (threatCount) threatCount.textContent = "LIVE";
        if (threatSource) threatSource.textContent = "Verified threat-intelligence telemetry dari ThreatFox; visual map menunjukkan pulse volume, bukan lokasi serangan presisi.";
        renderThreatFeed(d.data, true);
        return;
      }
      throw new Error("demo");
    } catch (e) {
      if (iocCount) iocCount.textContent = "SIM";
      if (malwareCount) malwareCount.textContent = "LIVE UI";
      if (sourceAge) sourceAge.textContent = "NOW";
      if (threatCount) threatCount.textContent = "DEMO";
      if (threatSource) threatSource.textContent = "Mode DEMO: animasi telemetry untuk presentasi UI. Aktifkan THREATFOX_AUTH_KEY di Vercel untuk feed terverifikasi.";
  ...
  if (threatMap && !reduced) {
    const ctx = threatMap.getContext("2d", { alpha: true });
    let W = 0, H = 0, dpr = 1, raf = 0, points = [];
    const hubs = [
      { x: .18, y: .43, c: "red" },
      { x: .37, y: .34, c: "cyan" },
      { x: .51, y: .29, c: "cyan" },
    ...
    const resize = () => {
      const r = threatMap.getBoundingClientRect();
      dpr = Math.min(devicePixelRatio || 1, 1.5);
      W = r.width; H = r.height;
      threatMap.width = Math.floor(W * dpr); threatMap.height = Math.floor(H * dpr);
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      points = hubs.map((p, i) => ({ ...p, phase: i * .8 }));
    };
    const color = p => p.c === "red" ? "220,38,38" : p.c === "amber" ? "180,83,9" : p.c === "green" ? "5,150,105" : "29,78,216";
    const draw = (t = 0) => {
      ...
      points.forEach((p, i) => {
        ...
        p.phase += .018;
        const pulse = (Math.sin(p.phase + t * .001) + 1) / 2;
        ctx.beginPath();
        ctx.arc(x, y, 3 + pulse * 3, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(${color(p)},${.22 + pulse * .2})`;
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x, y, 8 + pulse * 10, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(${color(p)},${.05 + pulse * .05})`;
        ctx.stroke();
      });
      points.forEach((a, i) => points.forEach((b, j) => {
        ...
        ctx.moveTo(ax, ay);
        ctx.lineTo(bx, by);
        ctx.strokeStyle = "rgba(29,78,216,.15)";
        ctx.stroke();
      }));
      raf = requestAnimationFrame(draw);
    };
    resize();
    addEventListener("resize", resize, { passive: true });
    new IntersectionObserver(es => { if (es[0].isIntersecting) { cancelAnimationFrame(raf); raf = requestAnimationFrame(draw); } }, { threshold: .05 }).observe(threatMap);
    raf = requestAnimationFrame(draw);
  }
